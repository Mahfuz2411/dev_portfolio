        ...
        <section className="min-h-screen relative overflow-hidden px-4 sm:px-8 py-4 sm:py-8 lg:py-16">
            ...
            <div className="max-w-6xl mx-auto relative z-10">
                ...
                <div className="space-y-8 mb-8">
                    {/* Profile Card */}
                    <Card className="p-6 sm:p-8 text-center border-2 border-slate-200 bg-white/80 backdrop-blur-sm">
                        <div className="relative w-40 h-40 sm:w-48 sm:h-48 mx-auto mb-6 rounded-full overflow-hidden border-4 border-indigo-200 shadow-xl">
                            <img 
                ...
                <div>
                    ...
                    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {skills.map((skillGroup, index) => (
                            <Card 
                                key={index}
                                className="p-6 border-2 border-slate-200 hover:border-indigo-300 bg-white/80 backdrop-blur-sm transition-all duration-300 hover:shadow-xl"
                            >
                                ...
                                <div className="space-y-2">
                                    {skillGroup.items.map((skill, idx) => (
                                        <div 
                                            key={idx}
                                            className="text-sm text-slate-700 py-1.5 px-3 bg-slate-100 rounded-md hover:bg-indigo-50 transition-colors"
                                        >
                                            {skill}
                                        </div>
                                    ))}
                ...
                <Card className="p-6 sm:p-8 border-2 border-slate-200 bg-white/80 backdrop-blur-sm mb-8">
                    ...
                    <div className="space-y-4">
                        {timeline.map((item, index) => {
                            ...
                            return (
                                <div 
                                    key={index}
                                    className="flex items-center gap-4 p-4 rounded-lg hover:bg-indigo-50 transition-colors duration-200"
                                >
